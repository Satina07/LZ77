...
public class Tag {
    ...
    public void setPosition(long position) {
        if (position >= Math.pow(2, bufferBitLength))
            throw new IllegalArgumentException("Заданная позиция не может поместиться в текущую длину бита");
        _position = position;
    }
    ...
    public void setLength(long length) {
        if (length >= Math.pow(2, bufferBitLength))
            throw new IllegalArgumentException("Заданная длина не может поместиться в текущую длину бита");
        _length = length;
    }
    ...
    public BitSet convertToBits() {
        // Тэг - это <Позиция , длина , следующий символ>
        //биты длины, биты длины, 8 (ASCII)
